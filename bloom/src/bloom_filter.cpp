...
double BloomFilter::search(const string &word) {
  if (k == 1) {
    int hash = sha256(word) % m;

    if (data[hash] == 0) {
      return 0;
    }
  } else if (k == 2) {
    int hash1 = sha256(word);
    int hash2 = sha512(word);
    int combined_hash = (hash1 + hash2) % m;
...
//     // Convert hash to an integer by taking the first 4 bytes and
//     interpreting them as a 32-bit integer uint32_t hash_int = *((uint32_t*)
//     hash);
...
//   SHA384(reinterpret_cast<const unsigned char *>(w.c_str()), w.length(),
//   sha384_hash); string sha384_str(reinterpret_cast<const char
//   *>(sha384_hash), SHA384_DIGEST_LENGTH); return hash<string>{}(sha384_str) %
//   m;
// }

// int BloomFilter::sha256(const string &w) {
//   unsigned char sha256_hash[SHA256_DIGEST_LENGTH];
//   SHA256(reinterpret_cast<const unsigned char *>(w.c_str()),
//   w.length(),sha256_hash); string sha256_str(reinterpret_cast<const char
//   *>(sha256_hash),SHA256_DIGEST_LENGTH); return hash<string>{}(sha256_str) %
//   m;
// }
